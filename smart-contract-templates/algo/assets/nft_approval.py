...
def approval_program():
    ...
    create_nft = Seq([
        ...
        # store id_h in scratchVar
        id_h.store(Concat(create_nft_id, Bytes("_h"))),
        App.globalPut(create_nft_id, data_ref),
        App.globalPut(id_h.load(), data_hash),

        # Int(0) represents the address of caller
        App.localPut(Int(0), create_nft_id, var_1),
        Return(is_creator)
    ])

    # this is only in the transfer condition
    transfer_nft_id = Txn.application_args[1]
