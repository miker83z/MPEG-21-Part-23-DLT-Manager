 ...
 * type.
 *
 * Extension of the OpenZeppelin EnumerableMap.
 */
library EnumerableMap {
        ...
        returns (bool, uint256)
    {
        (bool success, bytes32 value) =
            _tryGet(map._inner, bytes32(uint256(uint160(key))));
    ...
    /**
     * @dev Returns all the key value tuples.  O(n).
     */
    function getAll(AddressToUintMap storage map)
        internal
        view
        returns (address[] memory, uint256[] memory)
    {
        uint256 l = _length(map._inner);
        require(l > 0, "Empty Map");
        address[] memory keys = new address[](l);
        uint256[] memory values = new uint256[](l);

        for (uint256 index = 0; index < l; index++) {
            (bytes32 key, bytes32 value) = _at(map._inner, index);
            keys[index] = address(uint160(uint256(key)));
            values[index] = uint256(value);
        }
